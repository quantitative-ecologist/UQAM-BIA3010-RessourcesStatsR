...
# Préparons nos table de données pour l'ANOVA

On calcule le nombre de captures par type de fragment forestier en utilisant la fonction `aggregate()`. L'argument `fun = length` réfère à la fonction qui est utilisée pour faire le calcul à travers les types de fragments.
...
On veut seulement les espèces trouvées par fragment
...
On recense le nombre d'espèces

```{r}
nbr_especes <- aggregate(donnees_capture1$capture, by = list(donnees_capture1$fragment), FUN = length)
names(nbr_especes)  <- c("fragment", "nbr_especes")
```


# On combine les données pour avoir une table pour l'analyse
nbrs <- merge(nbr_captures, nbr_especes, by = "fragment")

# On combine aux données de superficie
